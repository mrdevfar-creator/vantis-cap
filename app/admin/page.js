...
import {
  collection, getDocs, doc, updateDoc, getDoc, orderBy, query,
  onSnapshot, where,
...
export default function AdminPage() {
  ...
  const fetchDeposits = async () => {
    try {
      ...
      setDeposits(snapshot.docs.map(d => ({ id: d.id, ...d.data() })));
  ...
  const handleApprove = async (deposit) => {
    ...
    try {
      ...
      await updateDoc(doc(db, "deposits", deposit.id), {
        status: "APPROVED",
        approvedAt: new Date().toLocaleString("en-BD", { timeZone: "Asia/Dhaka" }),
      });
        ...
        try {
          ...
          if (referrerSnap.exists()) {
            ...
            if (commission > 0) {
              ...
              const updatedReferrals = (referrerData.referrals || []).map(r =>
                r.userId === deposit.userId
                  ? { ...r, commission: (r.commission || 0) + commission, commissionPaidAt: now }
                  : r
              );
              const newTotalCommission = updatedReferrals.reduce((s, r) => s + (r.commission || 0), 0);
  ...
  const handleReject = async (depositId) => {
    ...
    try {
      await updateDoc(doc(db, "deposits", depositId), {
        status: "REJECTED",
        rejectedAt: new Date().toLocaleString("en-BD", { timeZone: "Asia/Dhaka" }),
      });
      await fetchDeposits();
  ...
  const fetchUsers = () => {
    ...
    const unsubscribe = onSnapshot(collection(db, "users"), (snapshot) => {
      ...
      setBalanceInputs(prev => {
        const next = { ...prev };
        data.forEach(u => { if (!(u.id in next)) next[u.id] = (u.balance || 0).toString(); });
        return next;
      });
      setWithdrawalInputs(prev => {
        const next = { ...prev };
        data.forEach(u => { if (!(u.id in next)) next[u.id] = (u.tradingAccount?.balance || 0).toString(); });
        return next;
      });
  ...
  const fetchWithdrawals = () => {
    ...
    const unsubscribe = onSnapshot(
      ...
      (err) => { console.error("withdrawals error:", err); setWithdrawalsLoading(false); }
    );
    return unsubscribe;
  };
  ...
  const handleWithdrawalAction = async (wId, action, withdrawal) => {
    ...
    try {
      await updateDoc(doc(db, "withdrawals", wId), {
        status: action,
        processedAt: new Date().toLocaleString("en-BD", { timeZone: "Asia/Dhaka" }),
      });
      if (action === "COMPLETED") {
        // Deduct from user balance and update totalWithdrawal
        const userRef = doc(db, "users", withdrawal.userId);
        const userSnap = await getDoc(userRef);
        if (userSnap.exists()) {
          const ud = userSnap.data();
          const now = new Date().toLocaleString("en-US", { timeZone: "Asia/Dhaka", hour: "2-digit", minute: "2-digit", hour12: true, month: "numeric", day: "numeric", year: "2-digit" });
          await updateDoc(userRef, {
  ...
  const handleBalanceUpdate = async (userId) => {
    ...
    if (isNaN(newBalance) || newBalance < 0) { alert("Enter a valid amount"); return; }
    ...
    try {
      ...
      setLastUpdated({ userEmail, field: "Current Balance", oldValue: oldBalance, newValue: newBalance, time: now });
    } catch (err) { alert("Error: " + err.message); }
    finally { setBalanceUpdating(null); }
  };
  ...
  const handleWithdrawalUpdate = async (userId) => {
    ...
    try {
      ...
      setUsers(prev => prev.map(u =>
        u.id === userId
          ? { ...u, totalWithdrawal: newTotal, lastWithdrawal: { amount, timestamp: now } }
          : u
      ));
      setLastUpdated({ userEmail, field: "Withdrawal +" + amount.toFixed(2), oldValue: currentTotal, newValue: newTotal, time: now });
    } catch (err) { alert("Error: " + err.message); }
    finally { setWithdrawalUpdating(null); }
  };

  // Update commission for a specific referred user
  const handleCommissionUpdate = async (referrerId, referredUserId, referredIndex) => {
    const amount = parseFloat(commissionInputs[`${referrerId}-${referredIndex}`] || 0);
    ...
    try {
      ...
      const totalCommission = updatedReferrals.reduce((sum, r) => sum + (r.commission || 0), 0);
      await updateDoc(userRef, {
        referrals: updatedReferrals,
      ...
      setUsers(prev => prev.map(u =>
        u.id === referrerId
          ? { ...u, referrals: updatedReferrals, totalReferralCommission: totalCommission }
          : u
      ));
    } catch (err) { alert("Error: " + err.message); }
    finally { setCommissionUpdating(null); }
  };
    ...
    <div className="min-h-screen bg-[#0A0D14] py-10 px-4">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          <button
            onClick={() => activeTab === "deposits" ? fetchDeposits() : fetchUsers()}
        ...
        <div className="flex gap-2 mb-8">
          {[
            { key: "deposits", label: "Deposits", badge: totalPending > 0 ? totalPending : null, badgeColor: "bg-amber-500/20 text-amber-400" },
            { key: "users", label: "Users", badge: users.length > 0 ? users.length : null, badgeColor: "bg-violet-500/20 text-violet-400" },
            { key: "withdrawals", label: "Withdrawals", badge: withdrawals.filter(w => w.status === "PENDING").length > 0 ? withdrawals.filter(w => w.status === "PENDING").length : null, badgeColor: "bg-rose-500/20 text-rose-400" },
          ].map(tab => (
            <button key={tab.key} onClick={() => handleTabSwitch(tab.key)}
              className={`flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-medium transition-all ${
                activeTab === tab.key ? "bg-white text-black" : "bg-white/5 border border-white/10 text-gray-400 hover:text-white"
              }`}>
              {tab.label}
              {tab.badge !== null && (
                <span className={`text-xs px-1.5 py-0.5 rounded-full ${activeTab === tab.key ? "bg-black/10 text-black" : tab.badgeColor}`}>
                  {tab.badge}
                </span>
              )}
            </button>
          ...
          <div>
            <div className="grid grid-cols-3 gap-4 mb-8">
              ...
              <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-xl p-5">
                ...
                <p className="text-emerald-400 text-2xl font-bold">{totalApproved}</p>
              </div>
              <div className="bg-blue-500/10 border border-blue-500/20 rounded-xl p-5">
                <p className="text-blue-400/70 text-xs mb-1">Total Approved Amount</p>
                <p className="text-blue-400 text-2xl font-bold">${totalAmount.toFixed(2)}</p>
              </div>
            </div>
              ...
              {["PENDING", "APPROVED", "REJECTED", "ALL"].map(tab => (
                <button key={tab} onClick={() => setFilter(tab)}
                  className={`px-4 py-2 rounded-xl text-xs font-medium transition-all ${
                    filter === tab ? "bg-white text-black" : "bg-white/5 border border-white/10 text-gray-400 hover:text-white"
                  }`}>
                  {tab}
                </button>
              ))}
            ...
            <div className="bg-white/[0.03] border border-white/[0.07] rounded-2xl overflow-hidden">
              {filtered.length === 0 ? (
                <div className="py-16 text-center text-gray-600">No deposits found</div>
              ) : (
                <div className="overflow-x-auto">
                  <table className="w-full">
                    <thead>
                      <tr className="border-b border-white/5">
                        {["User", "Amount", "TXID", "Time", "Status", "Action"].map(h => (
                          <th key={h} className="text-left text-gray-500 text-xs font-medium px-6 py-4">{h}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody>
                      {filtered.map(deposit => (
                        <tr key={deposit.id} className="border-b border-white/5 last:border-0 hover:bg-white/[0.02]">
                          <td className="px-6 py-4">
                            <p className="text-white text-sm">{deposit.userEmail}</p>
                            <p className="text-gray-600 text-xs">{deposit.userId?.slice(0, 8)}...</p>
                          </td>
                          <td className="px-6 py-4">
                            <p className="text-emerald-400 font-bold">${deposit.amount}</p>
                            <p className="text-gray-600 text-xs">USDT TRC20</p>
                          </td>
                          <td className="px-6 py-4">
                            <p className="text-gray-300 text-xs font-mono max-w-[160px] truncate" title={deposit.txId}>{deposit.txId}</p>
                          </td>
                          <td className="px-6 py-4">
                            <p className="text-gray-400 text-xs">{deposit.createdAt}</p>
                          </td>
                          <td className="px-6 py-4">
                            <span className={`text-xs font-medium px-2.5 py-1 rounded-full ${
                              deposit.status === "PENDING" ? "bg-amber-500/15 text-amber-400"
                              : deposit.status === "APPROVED" ? "bg-emerald-500/15 text-emerald-400"
                          ...
                          <td className="px-6 py-4">
                            {deposit.status === "PENDING" ? (
                              <div className="flex gap-2">
                                <button onClick={() => handleApprove(deposit)} disabled={updating === deposit.id}
                                ...
                                <button onClick={() => handleReject(deposit.id)} disabled={updating === deposit.id}
          ...
          <div>
            ...
            {lastUpdated && (
              <div className="flex items-center gap-4 mb-5 p-4 bg-amber-500/8 border border-amber-500/20 rounded-2xl">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-amber-400 text-xs font-semibold mb-0.5">Last Updated</p>
                  <p className="text-white text-sm font-medium truncate">{lastUpdated.userEmail}</p>
                  <p className="text-gray-500 text-xs">
                    {lastUpdated.field}: <span className="text-rose-400">${lastUpdated.oldValue.toFixed(2)}</span>
                    {" → "}
                    <span className="text-emerald-400">${lastUpdated.newValue.toFixed(2)}</span>
                    {" · "}
                    {lastUpdated.time}
                  </p>
                </div>
                <button onClick={() => setLastUpdated(null)} className="text-gray-600 hover:text-gray-400 transition-colors shrink-0">
            ...
            <div className="flex items-center gap-4 mb-6">
              ...
              <div className="bg-violet-500/10 border border-violet-500/20 rounded-xl px-5 py-3 shrink-0">
                ...
                <p className="text-violet-400 text-xl font-bold">{users.length}</p>
              </div>
            </div>
              ...
              <div className="bg-white/[0.03] border border-white/[0.07] rounded-2xl overflow-hidden">
                <div className="overflow-x-auto">
                  <table className="w-full">
                    <thead>
                      <tr className="border-b border-white/5">
                        {["User", "Phone", "Balance", "Set Balance", "Withdrawn", "Add Withdrawal", "Ref Code", "Referred By", "Referrals", "Status", "Joined"].map(h => (
                          <th key={h} className="text-left text-gray-500 text-xs font-medium px-5 py-4 whitespace-nowrap">{h}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody>
                      {filteredUsers.map(user => (
                        <tr key={user.id} className="border-b border-white/5 last:border-0 hover:bg-white/[0.02]">
                          ...
                          {/* Phone */}
                          <td className="px-5 py-4">
                            <p className="text-gray-400 text-sm whitespace-nowrap">{user.phone || "—"}</p>
                          </td>
                          {/* Current balance */}
                          <td className="px-5 py-4">
                            <p className="text-amber-400 font-bold">${(user.balance || 0).toFixed(2)}</p>
                          </td>
                          {/* Set current balance */}
                          <td className="px-5 py-4">
                            <div className="flex items-center gap-2">
                              <input
                                type="number"
                                value={balanceInputs[user.id] ?? (user.balance || 0)}
                          ...
                          <td className="px-5 py-4">
                            <div>
                              <p className="text-sky-400 font-bold">${(user.totalWithdrawal || 0).toFixed(2)}</p>
                              {user.lastWithdrawal && (
                                <p className="text-gray-600 text-[10px] mt-0.5">Last: ${user.lastWithdrawal.amount?.toFixed(2)} · {user.lastWithdrawal.timestamp}</p>
                              )}
                            </div>
                          </td>
                          ...
                          <td className="px-5 py-4">
                            <div className="flex items-center gap-2">
                              <input
                                type="number"
                                value={withdrawalInputs[user.id] ?? (user.tradingAccount?.balance || 0)}
                                onChange={e => setWithdrawalInputs(prev => ({ ...prev, [user.id]: e.target.value }))}
                          ...
                          {/* Referral Code */}
                          <td className="px-5 py-4">
                            <p className="text-amber-400 text-xs font-mono font-bold">{user.referralCode || "—"}</p>
                          </td>
                          {/* Referred By */}
                          <td className="px-5 py-4">
                            <p className="text-gray-400 text-xs font-mono">{user.referredBy || "—"}</p>
                          </td>
                            ...
                            {user.referrals?.length > 0 ? (
                              <div className="flex flex-col gap-1">
                                ...
                                <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-full w-fit ${
                                  user.referrals.length >= 11 ? "bg-yellow-500/20 text-yellow-400" :
                                  user.referrals.length >= 6  ? "bg-slate-400/20 text-slate-300" :
        ...
        <div>
          ...
          <div className="grid grid-cols-3 gap-4 mb-6">
            <div className="bg-rose-500/10 border border-rose-500/20 rounded-xl p-5">
              ...
              <p className="text-rose-400 text-2xl font-bold">{withdrawals.filter(w => w.status === "PENDING").length}</p>
            </div>
            <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-xl p-5">
              <p className="text-emerald-400/70 text-xs mb-1">Completed</p>
              <p className="text-emerald-400 text-2xl font-bold">{withdrawals.filter(w => w.status === "COMPLETED").length}</p>
            </div>
            <div className="bg-blue-500/10 border border-blue-500/20 rounded-xl p-5">
              <p className="text-blue-400/70 text-xs mb-1">Total Paid Out</p>
              <p className="text-blue-400 text-2xl font-bold">${withdrawals.filter(w => w.status === "COMPLETED").reduce((s, w) => s + (w.amount || 0), 0).toFixed(2)}</p>
            </div>
          </div>
          ...
          <div className="flex gap-2 mb-6">
            {["PENDING", "COMPLETED", "REJECTED", "ALL"].map(f => (
              <button key={f} onClick={() => setWFilter(f)}
                className={`px-4 py-2 rounded-xl text-xs font-medium transition-all ${
                  wFilter === f ? "bg-white text-black" : "bg-white/5 border border-white/10 text-gray-400 hover:text-white"
                }`}>{f}</button>
            ))}
          </div>
          ...
          {withdrawalsLoading ? (
            ...
            <div className="bg-white/[0.03] border border-white/[0.07] rounded-2xl overflow-hidden">
              {withdrawals.filter(w => wFilter === "ALL" || w.status === wFilter).length === 0 ? (
                <div className="py-16 text-center text-gray-600">No withdrawal requests found</div>
              ) : (
                <div className="overflow-x-auto">
                  <table className="w-full">
                    <thead>
                      <tr className="border-b border-white/5">
                        {["User", "Amount", "Address", "Requested At", "Status", "Action"].map(h => (
                          <th key={h} className="text-left text-gray-500 text-xs font-medium px-6 py-4">{h}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody>
                      {withdrawals.filter(w => wFilter === "ALL" || w.status === wFilter).map(w => (
                        <tr key={w.id} className="border-b border-white/5 last:border-0 hover:bg-white/[0.02]">
                          <td className="px-6 py-4">
                            <p className="text-white text-sm">{w.userName || w.userEmail}</p>
                            <p className="text-gray-600 text-xs">{w.userEmail}</p>
                          </td>
                          <td className="px-6 py-4">
                            <p className="text-rose-400 font-bold">${(w.amount || 0).toFixed(2)}</p>
                            <p className="text-gray-600 text-xs">USDT TRC20</p>
                          </td>
                          <td className="px-6 py-4">
                            <p className="text-gray-300 text-xs font-mono max-w-[160px] truncate" title={w.address}>{w.address}</p>
                          </td>
                          <td className="px-6 py-4">
                            <p className="text-gray-400 text-xs">{w.requestedAt || w.createdAt?.slice(0, 10)}</p>
                          </td>
                          <td className="px-6 py-4">
                            <span className={`text-xs font-medium px-2.5 py-1 rounded-full ${
                          ...
                          <td className="px-6 py-4">
                            {w.status === "PENDING" ? (
                              <div className="flex gap-2">
                                <button onClick={() => handleWithdrawalAction(w.id, "COMPLETED", w)} disabled={wUpdating === w.id}
                                  className="px-3 py-1.5 bg-emerald-500/15 text-emerald-400 border border-emerald-500/20 text-xs font-medium rounded-lg hover:bg-emerald-500/25 transition-colors disabled:opacity-50">
                                  {wUpdating === w.id ? "..." : "Complete"}
                                </button>
                                <button onClick={() => handleWithdrawalAction(w.id, "REJECTED", w)} disabled={wUpdating === w.id}
                                  className="px-3 py-1.5 bg-red-500/15 text-red-400 border border-red-500/20 text-xs font-medium rounded-lg hover:bg-red-500/25 transition-colors disabled:opacity-50">
                                  Reject
                                </button>
                              </div>
      ...
      {referralModal && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center px-4 pb-4 sm:pb-0">
          <div className="absolute inset-0 bg-black/60 backdrop-blur-sm" onClick={() => setReferralModal(null)} />
          <div className="relative w-full max-w-lg bg-[#0F1318] border border-white/[0.08] rounded-3xl shadow-2xl overflow-hidden">
            ...
            <div className="flex items-center justify-between px-6 py-5 border-b border-white/[0.07]">
              <div className="flex items-center gap-3">
                <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-violet-400 to-purple-500 flex items-center justify-center text-white text-sm font-bold shrink-0">
                  {referralModal.name ? referralModal.name.slice(0,2).toUpperCase() : referralModal.email?.slice(0,2).toUpperCase()}
                </div>
                <div>
                  <p className="text-white font-semibold text-sm">{referralModal.name || referralModal.email}</p>
                  <p className="text-gray-500 text-xs">Referral Code: <span className="text-amber-400 font-mono">{referralModal.referralCode}</span></p>
                </div>
              </div>
              <button onClick={() => setReferralModal(null)} className="text-gray-600 hover:text-gray-300 transition-colors">
            ...
            <div className="grid grid-cols-3 gap-3 px-6 py-4 border-b border-white/[0.07]">
              <div className="text-center">
                <p className="text-violet-400 text-2xl font-bold">{referralModal.referrals?.length || 0}</p>
                <p className="text-gray-600 text-xs mt-0.5">Total Referrals</p>
              </div>
              <div className="text-center">
                <p className="text-emerald-400 text-2xl font-bold">${(referralModal.totalReferralCommission || 0).toFixed(2)}</p>
                <p className="text-gray-600 text-xs mt-0.5">Total Commission</p>
              </div>
              <div className="text-center">
                <p className="text-emerald-400 text-sm font-mono font-bold mt-1">{referralModal.referredBy || "—"}</p>
            ...
            <div className="max-h-72 overflow-y-auto">
              {referralModal.referrals?.length === 0 ? (
                <div className="py-10 text-center text-gray-600 text-sm">No referrals yet</div>
              ) : (
                <div className="divide-y divide-white/[0.05]">
                  {referralModal.referrals?.map((r, i) => (
                    <div key={i} className="flex items-center gap-3 px-6 py-4 hover:bg-white/[0.02]">
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        {r.commission > 0 && (
                          <span className="text-emerald-400 text-xs font-bold">${r.commission?.toFixed(2)}</span>
                        )}
                        <input
                          type="number"
                          placeholder="0.00"
                          value={commissionInputs[`${referralModal.id}-${i}`] ?? (r.commission || "")}
                          onChange={e => setCommissionInputs(prev => ({ ...prev, [`${referralModal.id}-${i}`]: e.target.value }))}
                        ...
                        />
                        <button
                          onClick={() => handleCommissionUpdate(referralModal.id, r.userId, i)}
                          disabled={commissionUpdating === `${referralModal.id}-${i}`}
                        ...
                        >
                          {commissionUpdating === `${referralModal.id}-${i}` ? "..." : "Set"}
                        </button>
                      </div>
                    </div>
                  ))}
            ...
            <div className="px-6 py-4 border-t border-white/[0.07] bg-white/[0.01]">
              <p className="text-gray-600 text-xs text-center">Set commission amount per referred user</p>
            </div>
          </div>
        </div>
      )}
